...
def test_execution(point, port, full_deck):
    ...
    try:
        ...
        print "---"
        raise
    print "blah "+str(port)+", "+str(point)
    return point

def initialize_metrics(seed, port_pos0, port_pos1, full_deck, pool):
    point_pos0 = {}
    point_pos1 = {}
    point_pos0['str'] = [-1] * 4
    point_pos1['str'] = [-1] * 4
    ...
    # t1 = multiprocessing.Process(target=test_execution, args=(point_pos0, port_pos0, full_deck,))
    # t2 = multiprocessing.Process(target=test_execution, args=(point_pos1, port_pos1, full_deck,))
    result0 = pool.apply_async(test_execution, (point_pos0, port_pos0, full_deck,))
    result1 = pool.apply_async(test_execution, (point_pos1, port_pos1, full_deck,))
    ...
    # t1.join()
    # t2.join()
    point_pos0 = result0.get()
    point_pos1 = result1.get()
    # print "AQUI0: "+str(result0.get())
    # print "AQUI1: "+str(result1.get())

    # print "blah "+str(port_pos0)+", "+str(point_pos0)
    # print "blah "+str(port_pos1)+", "+str(point_pos1)
...
if __name__ == "__main__":
    ...
    path = "hand_types_temp_test4/pstr_ostr"
    index = 3
    indeces = 9
    ...
    for seed in range(1, 100):
        point_pos0, point_pos1 = initialize_metrics(seed, port0, port1, full_deck, pool)
        point_pos0['id'] = seed
        point_pos1['id'] = seed
